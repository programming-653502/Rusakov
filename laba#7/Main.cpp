...
void __fastcall TFMain::FormCreate(TObject *Sender)
{
	...
	if (!FileExists(Dir))
	{
		ofstream File("e:\\LABI 2 sem\\Prog\\laba#7\\Debug\\Win32\\Accountability.txt");
		File.close();
...
void __fastcall TFMain::BMainInfoClick(TObject *Sender)
{
	TStringList *list =  new TStringList;
	list->LoadFromFile("e:\\LABI 2 sem\\Prog\\laba#7\\Debug\\Win32\\Accountability.txt");
	String S = "Кол-во книг в библиотеке: " +list->Strings[0]+"\nОбщяя стоймость книг: "+list->Strings[1]
	+"\nКол-во книг на руках: "+list->Strings[2]+"\nКол-во читателей: "+list->Strings[3];
	ShowMessage(S);
	delete list;
}
//---------------------------------------------------------------------------
